...
serve_read(envid_t envid, union Fsipc *ipc)
{
	...
	if ((bytes_read = file_read(o->o_file, (void*) ret->ret_buf, req->req_n, o->o_fd->fd_offset)) < 0)
		return bytes_read;
...
serve_write(envid_t envid, struct Fsreq_write *req)
{
	...
	int r;
	int bytes_written;
	/*struct Fsreq_set_size req_s;

	req_s.req_fileid = req->req_fileid;
	req_s.req_size = req->req_n;*/

	if ((r = openfile_lookup(envid, req->req_fileid, &o)) < 0)
		return r;

	/*if ((r = serve_set_size(envid, &req_s)) < 0)
		return r;*/
	
	if (o->o_file->f_size - o->o_fd->fd_offset < req->req_n) {
		if ((r = file_set_size(o->o_file, o->o_file->f_size + o->o_fd->fd_offset + req->req_n)) < 0)
			return r;
	}
