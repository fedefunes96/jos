...
trap_init(void)
{
	...
	SETGATE(idt[T_NMI], 0, GD_KT, nonMaskableInterrupt, 0);
	SETGATE(idt[T_BRKPT], 1, GD_KT, breakpoint_t, 3);
	SETGATE(idt[T_OFLOW], 1, GD_KT, overflow, 0);
	SETGATE(idt[T_BOUND], 0, GD_KT, BOUNDRangeExceeded, 0);
	SETGATE(idt[T_ILLOP], 0, GD_KT, invalidOpcode, 0);
	SETGATE(idt[T_DEVICE], 0, GD_KT, deviceNotAvailable, 0);
	SETGATE(idt[T_DBLFLT], 0, GD_KT, doubleFault, 0);
	SETGATE(idt[T_TSS], 0, GD_KT, invalidTSS, 0);
	SETGATE(idt[T_SEGNP], 0, GD_KT, segmentNotPresent, 0);
	SETGATE(idt[T_STACK], 0, GD_KT, stackFault, 0);
	SETGATE(idt[T_GPFLT], 0, GD_KT, generalProtection, 0);
	SETGATE(idt[T_PGFLT], 0, GD_KT, pageFault, 0);
	SETGATE(idt[T_FPERR], 0, GD_KT, x87FPUFloatingPointError, 0);
	SETGATE(idt[T_ALIGN], 0, GD_KT, alignmentCheck, 0);
