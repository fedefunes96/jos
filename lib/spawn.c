...
copy_shared_pages(envid_t child)
{
	...
	for (addr = 0; addr < (uint8_t*) UTOP; addr += PGSIZE) {
		if ((uvpd[PDX(addr)] & PTE_P) && (uvpt[PGNUM(addr)] & PTE_P) && (uvpt[PGNUM(addr)] & PTE_SHARE)) {
			if ((r = sys_page_map(0, (void*) addr, child, (void*) addr, uvpt[PGNUM(addr)] & (PTE_SYSCALL | PTE_SHARE)) < 0))
				return r;
		}		
	}
		/*pde = (pde_t *) (PGADDR(PDX(uvpd), PTX(uvpd), (PDX(addr) * sizeof(pde_t))));

		if ((*pde) & PTE_P) {
			pte = (pte_t *) (PGADDR(PDX(uvpt), PDX(addr), (PTX(addr) * sizeof(pte_t))));
